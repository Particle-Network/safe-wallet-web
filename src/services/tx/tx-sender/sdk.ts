import type { WalletAPI } from '@/components/safe-apps/types'
import { getSafeSDK } from '@/hooks/coreSDK/safeCoreSDK'
import type { ConnectedWallet } from '@/hooks/wallets/useOnboard'
import { getConnectedWallet, getOnboardStore } from '@/hooks/wallets/useOnboard'
...
export const switchWalletChain = async (onboard: WalletAPI, chainId: string): Promise<ConnectedWallet | null> => {
...
export const assertWalletChain = async (onboard: WalletAPI, chainId: string): Promise<ConnectedWallet> => {
 ...
 */
export const getUncheckedSafeSDK = async (onboard: WalletAPI, chainId: SafeInfo['chainId']): Promise<Safe> => {
...
export const getSafeSDKWithSigner = async (onboard: WalletAPI, chainId: SafeInfo['chainId']): Promise<Safe> => {
  const signer = await getAssertedChainSigner(onboard, chainId)
  const sdk = getAndValidateSafeSDK()
